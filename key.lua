--==================================================--
-- RSQ KEY SYSTEM — FULL LOCAL SCRIPT (FULLY UPDATED & FIXED)
--==================================================--
...
local function fetchDataWithRetry()
    for attempt = 1, 3 do
        ...
        if ok and res and res.record then
            ...
            if res.record.keys then
                ...
                for k, v in pairs(res.record.keys) do
                    ...
                    if keyCount <= 5 then -- Limit output
                        print(string.format("[RSQ] Key %d: %s -> UserID: %s, Generated by: %s", 
                            keyCount, k, tostring(v.rbx), tostring(v.generatedBy)))
                    end
                end
            ...
            -- Debug: Check for current user's keys
            if res.record.keys then
                print("[RSQ] Looking for keys for UserID:", USER_ID)
                local userKeyCount = 0
                for k, v in pairs(res.record.keys) do
                    if tostring(v.rbx) == USER_ID then
                        userKeyCount = userKeyCount + 1
                        print(string.format("[RSQ] Found key for user: %s (generated by: %s)", 
                            k, tostring(v.generatedBy)))
                    end
                end
                print("[RSQ] Total keys for current user:", userKeyCount)
            end
...
local function validate(keyToVerify, skipFetch)
    ...
    if not data then 
        print("[RSQ] Failed to fetch data")
        return false, "Connection Error" 
    end

    -- Ban Logic
    if data.bans then
        if data.bans[USER_NAME] then
            kickBanned(data.bans[USER_NAME].reason)
            return false, "Banned"
        elseif data.bans[USER_ID] then
            kickBanned(data.bans[USER_ID].reason)
            return false, "Banned"
        end
    end
    ...
    for storedKey, keyData in pairs(data.keys) do
        if storedKey == keyToVerify then
            entry = keyData
            foundKey = storedKey
            print("[RSQ] Found key in database!")
            break
        end
    end
    ...
    if not entry then 
        ...
        print("[RSQ] Key not found in database. Available keys:")
        local keyList = {}
        for k, v in pairs(data.keys) do
            table.insert(keyList, k)
        end
        print("[RSQ] " .. table.concat(keyList, ", "))
        return false, "❌ Invalid Key" 
    end
    
    -- Check user ID match
    local keyUserId = tostring(entry.rbx)
    print("[RSQ] Key's user ID:", keyUserId, "Current user ID:", USER_ID)
    
    if keyUserId ~= USER_ID then 
    ...
    if entry.exp ~= "INF" then
        ...
        if expTime then
            ...
            print("[RSQ] Key expires at:", expTime, "Current time:", currentTime)
...
local function createGroupRequirementNotification()
    ...
    refreshBtn.MouseButton1Click:Connect(function()
        ...
        if isInGroup then
            ...
            if hasSavedKey then
                print("[RSQ] Loading saved key:", CurrentKey)
...
--==================================================--
-- INITIAL KEY GUI (WITH GROUP CHECK) - UPDATED
--==================================================--
local function showKeyGUI()
    ...
    -- Button events - UPDATED WITH DEBUG LOGGING
    unlock.MouseButton1Click:Connect(function()
        ...
        print("[RSQ] Current User Name:", USER_NAME)
...
local function initializeWithGroupCheck()
    ...
    if hasSavedKey then
        ...
        print("[RSQ] Checking saved key:", CurrentKey)
        local ok, res = validate(CurrentKey, false)
        if ok then
...
--==================================================--
-- SECURITY LOOPS (HIGH FREQUENCY) - UPDATED
--==================================================--
task.spawn(function()
    while true do
